...
const moves =
{
  adventurer: function(gameData, helpers)
  {
    ...
    if (neighbourEnemies === 4)
      return helpers.weakestNeighbourDirection(localArea);

    if (   neighbourEnemies === 1
        && helpers.isSingleNeighbourEnemyKillable(gameData,
                                                  weakestNeighbourEnemy.tile))
    {
      return weakestNeighbourEnemy.direction;
    }

    if (neighbourEnemies > 0 && weakestNeighbourEnemy.health <= me.health)
      return weakestNeighbourEnemy.direction;

    if (me.health <= 60 && localArea.myTile.possibilities.directionToWell)
      return localArea.myTile.possibilities.directionToWell;

    if (neighbourFriends > 0 && weakestNeighbourFriend.health < 100)
    ...
    if (typeof nearestWeakerEnemy === 'undefined' && me.health < 100)
      return helpers.findNearestHealthWell(gameData);
